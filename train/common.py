...
<<<<<<< HEAD
    def _extract_early_stop_score(trainer) -> float | None:
        ...
        preferred = ["metrics/mAP50-95(B)", "metrics/mAP50(B)"]
        for key in preferred:
            fv = _safe_float(metrics.get(key))
            if fv is not None:
                return fv
    ...
    def _get_early_stop_patience(trainer) -> int:
    ...
    def _update_early_stop_state(trainer) -> None:
        patience = _get_early_stop_patience(trainer)
        if patience <= 0:
            return

        score = _extract_early_stop_score(trainer)
        if score is None:
            return

        state = getattr(trainer, "_jomoo_early_stop_state", None)
        if not isinstance(state, dict):
            state = {"best_score": None, "epochs_no_improve": 0}
            trainer._jomoo_early_stop_state = state

        best_score = state.get("best_score")
        if best_score is None or score > float(best_score) + 1e-9:
            state["best_score"] = score
            state["epochs_no_improve"] = 0
            return

        state["epochs_no_improve"] = int(state.get("epochs_no_improve", 0)) + 1
        if state["epochs_no_improve"] >= patience:
            trainer.stop = True
            print(
                "[早停触发]",
                f"patience={patience}",
                f"best={float(best_score):.6f}",
                f"current={score:.6f}",
            )
...
<<<<<<< HEAD
    ...
    resolved_weights = _resolve_cli_path(config.weights)
    weight_source = str(resolved_weights) if resolved_weights.exists() else config.weights

    if not resolved_weights.exists() and not Path(config.weights).is_absolute():
        print(f"[提示] 未在项目根找到权重 {resolved_weights}，将按 Ultralytics 默认行为处理：{config.weights}")
